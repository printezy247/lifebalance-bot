...
def build_suggestion_prompt(user_context):
    return f"""
You are a wise, kind life coach helping {user_context['name']} balance work and family. Based on:
...
Advances a meaningful goal (work OR personal)
Respects their energy level (e.g., don't suggest deep work if energy=2)
Includes a tiny joy element (e.g., "Listen to favorite podcast while doing X")
Takes < 90 mins (respects attention span)
Is phrased warmly: "How about trying [task]? It could help you feel [benefit]."
Respond ONLY with the suggestion — no extra text. """.strip()

def build_planning_prompt(goal): return f""" You are a project planning expert. Break this goal into:
...
Format as: 🎯 Goal: [goal] 📋 Plan:

[Subtask] (~[time])
Blocker prep: [action]
[Subtask] (~[time]) ... 💡 Tip: [One pro tip for success] 🎉 Celebrate: [Small reward idea]
Be practical, not idealistic. If goal is too big, suggest FIRST step only. """.strip()

--- BOT COMMANDS ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user await update.message.reply_html( f"Hi {user.first_name}! 👋 I’m <b>LifeBalance</b> — your friendly planner for work, family, and YOU.\n\n" "Try these:\n" "• /add [task] [time] → Add a task (e.g., /add Call mom 7pm)\n" "• /today → See your priorities\n" "• /done [task_id] → Mark task as complete\n" "• /snooze [task_id] → Delay task by 30 mins\n" "• /ai suggest → Get AI-powered task suggestion\n" "• /plan [goal] → Break goal into steps\n" "• /assist [task_id] → Get coaching tip mid-task\n" "• /reflect → Weekly reflection (every Sunday)\n" "• /help → See all commands\n\n" "Remember: This is about harmony, not perfection. 💛" )

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text( "/add <task> <time> → Add task (time formats: '3pm', '15:00', 'tomorrow 9am')\n" "/today → View today’s tasks\n" "/done <task_id> → Mark task as complete\n" "/snooze <task_id> → Delay task by 30 mins\n" "/ai suggest → Get AI-powered task suggestion\n" "/plan [goal] → Break goal into steps\n" "/assist [task_id] → Get coaching tip mid-task\n" "/reflect → Weekly reflection (run every Sunday)\n" "/help → Show this menu" )

async def add_task(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data: data[user_id] = {"tasks": [], "reminders": []}
...
async def show_today(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data or not data[user_id]["tasks"]: await update.message.reply_text("Your plate is clean! 🍽️ Add something with /add") return
...
async def complete_task(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data: await update.message.reply_text("No tasks yet! Use /add first.") return
...
async def snooze_task(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data: await update.message.reply_text("No tasks to snooze!") return
   ...
   await update.message.reply_text("Task not found or already done.")
async def weekly_reflect(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data: data[user_id] = {"tasks": [], "reflections": []}
   ...
   save_data(data)
async def ai_suggest(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() user_data = data.get(user_id, {"tasks": [], "reflections": []})
...
async def plan_goal(update: Update, context: ContextTypes.DEFAULT_TYPE): if not context.args: await update.message.reply_text("Usage: /plan [your goal]\nExample: /plan Prepare quarterly presentation") return
...
async def assist_task(update: Update, context: ContextTypes.DEFAULT_TYPE): if not context.args: await update.message.reply_text("Usage: /assist [task_id]\nSee /today for task IDs") return
   ...
   prompt = f"""
You are a supportive task coach. User is about to work on: "{task['text']}" Give ONE specific, actionable tip to help them start or overcome a common hurdle. Examples:
...
For work: "Set a 25-min timer - focus ONLY on this until it dings" Keep it under 2 sentences. Be warm and practical. """.strip()
...
async def reschedule_task(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) data = load_data() if user_id not in data: await update.message.reply_text("No tasks! Use /add first.") return
...
async def handle_reflection(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = str(update.effective_user.id) if not context.user_data.get("awaiting_reflection"): return
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = str(query.from_user.id) data = load_data()
 ...
 save_data(data)
def main(): application = Application.builder().token(os.getenv("BOT_TOKEN")).build()
...
async def check_reminders(context: ContextTypes.DEFAULT_TYPE): now = datetime.datetime.now() data = load_data()
